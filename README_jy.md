...
- 코드를 전체적으로 cpu 전용 코드와 모델로 바꾸어서 그런지 최초 질문 생성 및 꼬리 질문 생성 질이 좋지는 않음(5에폭으로만 돌려도 stage1 학습 시 한 시간 소요되어 긴 에폭으로는 돌리지 못했음)
- 또한 최초질문 생성 시 "수협은행에 입사하고자 하는 이유를 설명하고, 입사 후 이루고자 하는 목표를 작성하여 주십시오." 이런식으로 특정 회사명이 포함되어 출력되는 것 같은 문제가 조금 있음 --> 근데 이건 학습을 오래 못 돌려서일 가능성도 있음,,
- cli에서는 아래와 같이 최초질문~꼬리질문에 대한 답변들 간의 모순성을 잡아줌 + 꼬리 질문 생성 시 앞선 모든 내용들을 참고하여 생성 + 적합 직무의 변화까지. 예를들어, 팀장으로써 팀을 1등으로 이끌었다. -> 1등을 해본 적은 없지만~~ ->모순성 탐지

```text
일관성 검사
  ⚠ 확인 필요
  - 꼬리답변 1과 현재 답변 사이의 NLI contradiction 점수가 높아 모순 가능성이 큽니다. (score=0.9997)
  - 전체 이전 답변과 현재 답변 사이의 NLI contradiction 점수가 높아 누적 맥락과 충돌할 가능성이 있습니다. (score=0.9996)
  보완 방향
  → 꼬리답변 1에서 말한 내용과 현재 답변의 역할·권한·기여도 표현이 함께 성립하는지 확인해 주세요.
  → 같은 경험을 말하고 있을 가능성이 높으므로, 역할·권한·기여도 표현이 충돌하지 않게 정리해 주세요.
  → 전체 답변 흐름에서 역할, 권한, 기여도, 시점을 한 번에 정리해 주세요.


=======================================================
  적합 직무 변화 추적
=======================================================
  Turn  0 | initial    | product  | intent  14.6/100
  Turn  1 | tail_1     | product  | intent   9.1/100
  Turn  2 | tail_2     | product  | intent  12.1/100
--------------------------------------------------------
  시작 직무 → 최종 직무 : product → product
  의도 반영도 변화       : 14.6 → 12.1 (-2.5)
  세션 내 TOP 직무       : product
  해석                  : 직무 방향과 점수가 비교적 안정적으로 유지됐습니다.
=======================================================
```
---
