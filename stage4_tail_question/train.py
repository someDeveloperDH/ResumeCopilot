...
def load_samples():
    ...
    if STAGE1_DATA.exists():
        ...
        samples += [format_stage1(r) for _, r in df1.iterrows()]
    ...
    if STAGE4_DATA.exists():
        ...
        samples += [format_stage4(r) for _, r in df4.iterrows()]
...
def main():
    ...
    train_texts, val_texts = load_samples()
    print(f"Train: {len(train_texts)}개 | Val: {len(val_texts)}개")
    print(f"모델 로딩: {MODEL_NAME} (CPU)")
