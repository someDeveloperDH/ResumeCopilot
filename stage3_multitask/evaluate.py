...
def main():
    ...
    r_mark = "계산 불가" if np.isnan(r) else pass_mark(r, _EVAL["pearson_threshold"])
    print(f"  Pearson r: {r:.4f}   (기준 ≥ {_EVAL['pearson_threshold']}) {r_mark}")
    print()
    ...
    results = {
        ...
        "regression": {"mae": float(mae), "rmse": float(rmse), "pearson_r": None if np.isnan(r) else float(r)},
        "classification": {"accuracy": float(acc), "f1_macro": float(f1), "precision": float(prec), "recall": float(rec)},
