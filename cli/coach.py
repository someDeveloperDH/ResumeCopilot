...
if TYPE_CHECKING:
    from agents.interviewer_agent import InterviewContext
...
def build_tail_question_prompt(ctx: "InterviewContext") -> str:
    context_block = ctx.conversation_context.strip() or f"[최초 답변]\n{ctx.answer}"
    note_block = f"\n[일관성 검사 메모]\n{ctx.consistency_note}\n" if ctx.consistency_note else ""
...
Yes/No로 답할 수 없어야 하며, 반드시 한국어 질문 한 문장으로만 출력하세요.
설명, 따옴표, 번호를 붙이지 말고 질문 문장만 출력하세요.

원본 질문: {ctx.question}
현재 의도 반영도: {ctx.intent_score:.0f}
현재 적합 직무: {ctx.job}
...
def clean_tail_question(raw: str) -> str:
    """Ollama 응답에서 꼬리질문 한 문장만 추출 (코드블록/머리말/따옴표 제거)."""
    text = re.sub(r"^```(?:\w+)?\s*|\s*```$", "", raw.strip(), flags=re.S).strip()

    for line in (l.strip() for l in text.splitlines()):
        if not line:
            continue
        line = re.sub(r"^(꼬리질문|다음\s*질문|질문)\s*[:：]\s*", "", line)
        line = line.strip(" -*\"'“”‘’")
        if line:
            return line
    return ""
