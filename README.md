...
'하루 1문답'은 자소서를 쓰기 어려워하는 IT 취준생들을 위하여 AI-Agent 시스템을 통한 CLI방식의 글쓰기 훈련도구 입니다.
하루 최소 1문제씩 주어지며 이용자는 간단한 본인의 이야기를 답변하고 그걸 기반으로 부족한 부분에 대하여
꼬리질문으로 문답이 진행 되어, 답변 작성에 대한 사고 확장을 목표로 하고 있습니다.
...
| 구성원 | 역할 |
|---|---|
| 박동혁 | 아키텍처 설계 · 데이터 구축 · 베이스라인 구축 |
| 안진영 | 아이디어 설계 및 고도화 · CLI 구축 · 모델 설정 |
| 이진태 | CLI 구축 및 고도화 · 모델 설정 · 평가 설계 |
